...
if ($items == 0) {
    ?>
    <tr>
        <td colspan="5" align="center" valign="middle" style="padding-top:15px"><h2>No Course Selected</h2></td>
    </tr>
<?php }
...
foreach (array_unique($_SESSION['module']) as $sess) {
    ...
    if (isset($sess)) {
        ...
        if ($code[0] == 'ee' || $code[0] == 'ss' or $code[0] == 'mm') {
            ...
            $r1 = mysqli_query($con, "select * from module_detail_9_10 where subject='$subject' and module_no='$module_no'");
            $r2 = mysqli_fetch_array($r1);
        } else {
            $r1 = mysqli_query($con, "select * from module_detail where subject='$subject' and module_no='$code[1]'");
            $r2 = mysqli_fetch_array($r1);
        }
        ?>
        <tr valign="top" align="center" height="40">
            ...
            if ($code[0] == 'mm' || $code[0] == 'ee' || $code[0] == 'ss') { ?>
                <td><img src="http://www.livecbse.com/delete.png" id="<?php echo 'delete' . $r2[0]; ?>"
...
if ($count_fe >= 1) {
    $count_fe_t = $count_fe * 10850;
    $count_fe_d = 0;
    $count_f1 = $count_fe_t - $count_fe_d;
    $service_tax_fe = $count_f1 * (18 / 100);
    $count_f_fe = $count_f1 + $service_tax_fe;
} else {
}

if ($count_f >= 1) {
    $count_f_t = $count_f * 9195;
    $count_f_d = 0;
    $count_f1 = $count_f_t - $count_f_d;
    $service_tax_f = $count_f1 * (18 / 100);
    $count_f_f = $count_f1 + $service_tax_f;
} else {
}

if ($count_f9 >= 1) {
    $count_f9_t = $count_f9 * 4767;
    $count_f9_d = 0;
    $count_f1 = $count_f9_t - $count_f9_d;
    $service_tax_f9 = $count_f1 * (18 / 100);
...
if ($count_b == 1) {
    $count_b_t = 3000;
    $service_tax_b = 3000 * (15 / 100);
    $count_b_f = $count_b_t + $service_tax_b;
    $count_b_d = 0;
} else if ($count_b == 2) {
    $count_b_t = 6000;
    $count_b_d = $count_b_t * (5 / 100);
    $count_f1 = $count_b_t - $count_b_d;
    $service_tax_b = $count_f1 * (15 / 100);
    $count_b_f = $count_f1 + $service_tax_b;
} else if ($count_b == 3) {
    $count_b_t = 9000;
    $count_b_d = $count_b_t * (10 / 100);
    $count_f1 = $count_b_t - $count_b_d;
    $service_tax_b = $count_f1 * (15 / 100);
    $count_b_f = $count_f1 + $service_tax_b;
} else if ($count_b >= 4) {
    $count_b_t = $count_b * 3000;
    $count_b_d = floor($count_b_t * (16.666666667 / 100));
    $count_f1 = $count_b_t - $count_b_d;
    $service_tax_b = $count_f1 * (15 / 100);
    $count_b_f = $count_f1 + $service_tax_b;
} else {
...
if ($count_e == 1) {
    $count_e_t = 3000;
    $service_tax_e = 3000 * (15 / 100);
    $count_e_f = $count_e_t + $service_tax_e;
    $count_e_d = 0;
} else if ($count_e == 2) {
    $count_e_t = 6000;
    $count_e_d = $count_e_t * (5 / 100);
    $count_f1 = $count_e_t - $count_e_d;
    $service_tax_e = $count_f1 * (15 / 100);
    $count_e_f = $count_f1 + $service_tax_e;
} else if ($count_e == 3) {
    $count_e_t = 9000;
    $count_e_d = $count_e_t * (10 / 100);
    $count_f1 = $count_e_t - $count_e_d;
    $service_tax_e = $count_f1 * (15 / 100);
    $count_e_f = $count_f1 + $service_tax_e;
} else if ($count_e >= 4) {
    $count_e_t = $count_e * 3000;
    $count_e_d = floor($count_e_t * (16.666666667 / 100));
    $count_f1 = $count_e_t - $count_e_d;
    $service_tax_e = $count_f1 * (15 / 100);
    $count_e_f = $count_f1 + $service_tax_e;
} else {
...
if ($count_ee == 1) {
    ...
    $service_tax_ee = $count_f1 * (15 / 100);
    ...
    $count_ee_d = floor($count_ee_t * (16.666666667 / 100));
    $count_f1 = $count_ee_t - $count_ee_d;
    $service_tax_ee = $count_f1 * (15 / 100);
    $count_ee_f = $count_f1 + $service_tax_ee;
} else {
...
if ($count_mm == 1) {
    $count_mm_t = 4767;
    $service_tax_mm = 4767 * (18 / 100);
    $count_mm_f = $count_mm_t + $service_tax_mm;
    $count_mm_d = 0;
} else if ($count_mm >= 2) {
    $count_mm_t = $count_mm * 4767;
    $count_mm_d = 0;
    $count_f1 = $count_mm_t - $count_mm_d;
    $service_tax_mm = $count_f1 * (18 / 100);
    ...
    $count_mm_d = floor($count_mm_t * (16.666666667 / 100));
    $count_f1 = $count_mm_t - $count_mm_d;
    $service_tax_mm = $count_f1 * (18 / 100);
    $count_mm_f = $count_f1 + $service_tax_mm;
} else {
...
if ($count_ss == 1) {
    $count_ss_t = 4767;
    $service_tax_ss = 4767 * (18 / 100);
    $count_ss_f = $count_ss_t + $service_tax_ss;
    $count_ss_d = 0;
} else if ($count_ss >= 2) {
    $count_ss_t = $count_ss * 4767;
    $count_ss_d = 0;
    ...
    $count_f1 = $count_ss_t - $count_ss_d;
    $service_tax_ss = $count_f1 * (18 / 100);
    $count_ss_f = $count_f1 + $service_tax_ss;
} else if ($count_ss >= 4) {
    $count_ss_t = $count_ss * 3750;
    $count_ss_d = $count_ss_t * (18 / 100);
    $count_f1 = $count_ss_t - $count_ss_d;
    $service_tax_ss = $count_f1 * (18 / 100);
    $count_ss_f = $count_f1 + $service_tax_ss;
} else {
...
if ($total != 0) {
    //echo '$'.$total.'$'.round($dis).'$'.round($fees).'$'.$items;
} else {
    //echo "$0$0$0$0";
}
?>
